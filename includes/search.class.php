...
class bsiSearch
{
    ...
    private function loadRoomTypes()
    {
        ...
        while ($currentrow = mysql_fetch_assoc($sql)) {
            array_push($this->roomType, array(
                'rtid' => $currentrow["roomtype_ID"],
                'rtname' => $currentrow["type_name"]
            ));
        }
    ...
    private function loadMultiCapacity()
    {
        ...
        while ($currentrow = mysql_fetch_assoc($sql)) {
            $this->multiCapacity[$currentrow["id"]] = array(
    ...
    public function getAvailableRooms($roomTypeId, $roomTypeName, $capcityid)
    {
        ...
        while ($currentrow = mysql_fetch_assoc($sql)) {
            ...
            array_push($searchresult['availablerooms'], array(
                'roomid' => $currentrow["room_ID"],
                'roomno' => $currentrow["room_no"]
            ));
            $tmpctr++;
        }
        mysql_free_result($sql);
        if ($tmpctr >= 1) {
            ...
            foreach ($totalDays[0] as $date2 => $val) {
                ...
                if (mysql_num_rows($sql_sp)) {
                    ...
                    $$h2 += $c999;
                    if ($row99['min_stay'] <= $minimum_night_flg) {
                        $specail_price_flag = 1;
                    }
                    $minimum_night_flg++;
                } else {
